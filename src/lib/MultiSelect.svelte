...
<div
  class="flex flex-wrap gap-1 input input-bordered min-h-[3rem] p-1 h-auto relative content-start {Object.keys(
    options
  ).length
...
>
  {#each values as value}
    <div
      class="flex items-center bg-base-300 rounded-lg py-2 pl-3 pr-9 h-10 relative"
    >
      <span>{value}</span>
      <button
        type="button"
